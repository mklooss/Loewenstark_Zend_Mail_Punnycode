...
class Zend_Mail extends Zend_Mime_Message
{
    ...
    */
    ...
    protected function _filterEmail($email)
    {
        ...
        $result = strtr($email, $rule);
        $emailParts = array_filter((array) explode('@', $result, 2));
        if (count($emailParts) == 2 && idn_to_ascii($emailParts[1]) !== $emailParts[1])
        {
            $result = $emailParts[0].'@'.idn_to_ascii($emailParts[1]);
        }
        return $result;
    }
